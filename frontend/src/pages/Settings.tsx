          ...
          <>
              ...
              <CardContent className="p-6">
                ...
                <div className="mt-6 grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input id="name" aria-label="Display name" data-testid="settings-name-input" value={name} onChange={e => setName(e.target.value)} className="border-white/10 bg-black/40" />
                    {nameError && <p className="text-xs text-red-400" role="alert">{nameError}</p>}
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="email">Email</Label>
                    <Input id="email" aria-label="Email" data-testid="settings-email-input" type="email" value={email} disabled className="border-white/10 bg-black/40 opacity-60" />
                  </div>
                </div>
            ...
            <Card className="border-white/10 bg-white/[0.03]">
              <CardContent className="flex flex-wrap items-center justify-between gap-3 p-6">
                ...
                <Button variant="outline" onClick={signOut} data-testid="settings-logout-button" className="gap-2 rounded-full border-red-500/30 bg-transparent text-red-300 hover:bg-red-500/10">
          ...
          <Card className="border-white/10 bg-white/[0.03] backdrop-blur-xl" data-testid="settings-security-section">
            <CardContent className="space-y-6 p-6">
              <div data-testid="settings-openrouter-section">
                ...
                {keyLoading ? (
                  <div className="mt-3 flex items-center gap-2 text-sm text-zinc-400"><Loader2 className="h-4 w-4 animate-spin text-violet-400" aria-hidden="true" />Checking connection…</div>
                ) : keyStatus?.connected ? (
                  <div className="mt-3 flex flex-wrap items-center gap-3">
                    <Badge className="gap-1.5 border-emerald-500/30 bg-emerald-500/15 px-3 py-1.5 text-emerald-300">
                    ...
                    <Button variant="outline" size="sm" onClick={removeKey} disabled={saving} data-testid="settings-disconnect-key-button" className="rounded-full border-red-500/30 bg-transparent text-red-300 hover:bg-red-500/10">
                  ...
                  <div className="mt-3 flex gap-2">
                    <div className="relative flex-1">
                      <Input aria-label="OpenRouter API key" data-testid="settings-apikey-input" type={showKey ? "text" : "password"} value={apiKey} onChange={e => setApiKey(e.target.value)} placeholder="sk-or-…" className="border-white/10 bg-black/40 pr-10 font-mono text-sm" />
