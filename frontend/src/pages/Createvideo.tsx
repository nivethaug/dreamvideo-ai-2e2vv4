  ...
  return (
    <div className="page-enter mx-auto w-full max-w-[1440px] px-4 py-8 md:px-8 md:py-10" data-testid="create-page">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className="text-xs uppercase tracking-[0.25em] text-violet-600/80">Create</p>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2" ref={modelMenuRef} data-testid="create-model-card">
                  <Label className="flex items-center gap-1.5 text-xs font-medium uppercase tracking-wider text-zinc-400">
                    <Layers className="h-3.5 w-3.5 text-violet-600" aria-hidden="true" />Model
                  </Label>
                  ...
                  {modelsError && <p className="text-[10px] leading-snug text-amber-600">{modelsError}</p>}
                </div>
                ...
                <div className="space-y-2" data-testid="create-style-card">
                  <Label htmlFor="style" className="flex items-center gap-1.5 text-xs font-medium uppercase tracking-wider text-zinc-400">
                    <Palette className="h-3.5 w-3.5 text-violet-600" aria-hidden="true" />Style
                  </Label>
                  <div className="relative">
                    <select
                      id="style"
                  ...
                  <p className="text-[10px] text-zinc-400">Visual treatment applied to the video.</p>
                </div>
              </div>
            ...
            <CardContent className="p-5">
              ...
              {mediaQuery.trim() === "" ? (
                ...
                <div className="flex items-center justify-center gap-2 py-12 text-sm text-zinc-400" aria-live="polite">
                  <Loader2 className="h-4 w-4 animate-spin text-violet-600" aria-hidden="true" />Searching Pexels…
                </div>
              ) : mediaError ? (
                <div className="flex flex-col items-center gap-2 py-12 text-center" role="alert">
                  {mediaError.startsWith("No results") ? <ImageOff className="h-6 w-6 text-zinc-500" aria-hidden="true" /> : <AlertTriangle className="h-6 w-6 text-amber-600" aria-hidden="true" />}
              ...
              <p className="mt-3 text-[11px] text-zinc-400">Stock videos from Pexels — search and preview reference footage.</p>
            </CardContent>
          </Card>
        </div>
            ...
            <div className="relative aspect-video" data-testid="create-canvas">
              {videoLive ? (
                ...
                <div className="flex h-full w-full flex-col items-center justify-center gap-2 bg-[#0b0b18] text-zinc-500">
                  {jobBusy ? <><Loader2 className="h-8 w-8 animate-spin text-violet-600" aria-hidden="true" /><p className="text-sm">{job?.status ?? "Queued"}…</p></>
              ...
              {job?.status === "Completed" && job.provider_url && urlExpired(job.expires_at) && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/80 p-6 text-center text-sm text-zinc-100" role="alert">
